...
function calculateEncounters() {
  ...
  let whichEncounterRoll = document.querySelector("#guide").checked
    ? defaultRules.guideRules.guideRoll
    : defaultRules.guideRules.noGuideRoll;
  let encounterChance =
    document.querySelector("#terrain").value == "road"
      ? defaultRules.encounterChances.roadChance
      : defaultRules.encounterChances.wildChance;
  let encounterHours = travelFlag
    ? parseFloat(glob.distance.value) > currentPace.milesPerDay
      ? Math.round(currentPace.milesPerDay / currentPace.milesPerHour)
      : Math.round(parseFloat(glob.distance.value) / currentPace.milesPerHour)
    : parseFloat(waitTime.value);
  let travelLog = document.getElementById("travelLog");
  let milesTravelled;
  // We know our parameters, time to check for encounters
  encounterInterval = 1.5;
  for (
    chunk = encounterInterval;
    chunk <= encounterHours;
    chunk += encounterInterval
  ) {
    glob.recentEncounters +=
      glob.recentEncounters <= 0 ? 0 : -1 / (24 / encounterInterval);
    // Roll to see if an encounter happens
    let encounterCheck = roll("1d20");
    if (
      encounterCheck >= encounterChance &&
      glob.recentEncounters < defaultRules.maxEncounters - 1
    ) {
      ...
      let encounter =
        document.querySelector("#time").value == "day"
          ? dayEncounters[roll(whichEncounterRoll)]
          : nightEncounters[roll(whichEncounterRoll)];
      let nameCheck = encounter.name;
      if (
        !(
          nameCheck == "Skeletal Rider" &&
          document.querySelector("#skelly").checked
        ) && // Encounter isn't Rider or the group hasn't already done rider
        !(
          nameCheck == "Will O' Wisp" &&
          document.querySelector("#willOWisp").checked
        ) && // Encounter isn't Wisp or the group hasn't already done wisp
        (!encounter.travelOnly || travelFlag)
      ) {
        // Group is either travelling, or the encounter is not travel-only
        glob.recentEncounters++;
        encounter.init;
        ...
        travelLog.innerHTML += travelFlag
          ? `Group travels ${milesTravelled} miles, then...\n`
          : `Group waits ${chunk} hours, then...\n`;
        travelLog.innerHTML += `The players encounter ${encounter.snippet}\n`;
        travelLog.innerHTML += travelFlag
          ? `The group has ${milesLeft} miles left to travel`
          : `The group has ${hoursLeft} hours left to wait`;
        ...
        perceptionField.innerHTML =
          encounter.perception == undefined ? "-" : encounter.perception;
  ...
  milesTravelled =
    parseFloat(glob.distance.value) > currentPace.milesPerDay
      ? currentPace.milesPerDay
      : glob.distance.value;
  travelLog.innerHTML += travelLog.innerHTML == "" ? "" : "\n";
  travelLog.innerHTML += travelFlag
    ? `Group travels ${milesTravelled} miles without incident`
    : `Group waits ${waitTime.value} hours without incident`;
  glob.distance.value = travelFlag
    ? parseFloat(glob.distance.value) > currentPace.milesPerDay
      ? parseFloat(glob.distance.value) - milesTravelled
      : ""
    : "";
  waitTime.value = "";
  travelLog.scrollTop = travelLog.scrollHeight;
...
const repeatEncounters = {
  bloodPine: {
    ...
    perception: 13,
    description:
      "One of the nearby pine trees shudders suddenly. You notice dark red sap dripping from its bark and needles, and a root covered in spikes slithers through the brush towards you.",
    rules: "",
    get init() {
  ...
  grave: {
    ...
    get init() {
      if (Math.random() >= 0.25) {
        ...
        this.rules =
          "Characters who dig up the grave find the skeletal remains of a human clad in rusted chain mail (a soldier). Among the bones lie corroded weapons.";
      } else {
        this.description =
          "You stumble upon an old grave. It's clearly been violated. All that's left is a shallow, mud-filled hole with dirt and rocks strewn around it and a few scattered bones within.";
        this.rules = "";
      }
      this.snippet = `a ${this.name}`;
    },
  ...
  trinket: {
    ...
    get init() {
      let trinket =
        trinketArray[Math.floor(Math.random() * trinketArray.length)];
  ...
  corpse: {
    ...
    get init() {
      switch (roll("1d6")) {
        ...
        case 2:
          this.rules =
            "The corpse belongs to a wolf killed by spears and crossbow bolts.";
          break;
        case 3:
        case 4:
        case 5:
  ...
  wolves: {
    ...
    get init() {
      this.rules = `Characters have a few minutes to steel themselves before these wolves attack. They heed the will of Strahd and can't be charmed or frightened. If the characters prepare accordingly, they may be able to hide from the wolves. (DC ${advantage(
        3
      )})`;
      this.number = roll(this.range);
      this.snippet = `${this.number} ${this.name}`;
  ...
  needleBlights: {
    ...
    get init() {
      ...
      this.number = roll(this.range);
      this.rules = `The woods crawl with needle blights that serve the evil druids of Barovia. If the characters are moving quietly and not carrying light sources, they can try to hide from these blights. (DC ${roll(
        "1d20-1"
      )})`;
      this.snippet = `${this.number} ${this.name}`;
    },
...
const dayEncounters = {
  ...
  6: {
    ...
    perception: 13,
    get init() {
      this.rules = `Evil druids left this trail. Following it in either direction leads to a spiked pit. A thin tarp made of twigs and pine needles conceals the pit, the bottom of which is lined with sharpened wooden stakes.</p><p><strong>Spiked Pit:</strong> This hidden pit trap is 3 ft across and 10 ft deep, and it has sharpened wooden spikes at the bottom. A creature falling into the pit takes ${roll(
        "2d10"
      )} (2d10) piercing damage from the spikes, ${roll(
        "3d8"
      )} (3d8) poison damage from the venom on the spikes, and ${roll(
        "1d6"
      )} (1d6) bludgeoning damage from the fall.`;
      this.snippet = `a ${this.name}`;
    },
  },
  7: {
  ...
  11: {
    ...
    get init() {
      if (Math.random() < 0.5) {
        ...
        this.rules = `This wereraven in raven form watches the characters from a distance.</p><p>The wereraven belongs to a secret order called the Keepers of the Feather. If the characters don't spot it, the wereraven shadows them for ${roll(
          "1d4"
        )} hours. At the end of that time, or anytime sooner if the characters attack it, the creature flies home to report what it has seen.</p><p>If the party has a second random encounter with a wereraven, this one presents itself to the characters as an ally and requests that they travel to the Blue Water Inn in Vallaki to meet "some new friends." It then flies off in the direction of the town.`;
...
const nightEncounters = {
  ...
  16: {
    ...
    get init() {
      this.rules = `These unfortunate Barovians fell prey to the evils of the land and now shamble from place to place as a ravenous mob. The characters may attempt to sneak past the group of undead (DC ${roll(
        "1d20-2"
      )})`;
      this.number = roll(this.range);
      this.snippet = `${this.number} ${this.name}`;
  ...
  18: {
    ...
    get init() {
      this.rules = `If the characters are moving quietly and not carrying light sources, they can try to hide from the Strahd zombies (DC ${roll(
        "1d20-2"
      )}). These undead soldiers once served as guards in Castle Ravenloft. They fled the castle after Strahd became a vampire but couldn't avoid their master's wrath. They still wear bits of tattered livery, and they attack the living on sight.`;
      this.number = roll(this.range);
      this.snippet = `${this.number} ${this.name}`;
    },
  },
  19: {
    ...
    get init() {
      this.rules = `This random encounter occurs only once. If it comes up again, treat the result as no encounter.</p><p>If the characters follow the flickering light, read:<aside class=readAloud>The torchlight flutters as it moves away from you, but you never lose sight of it. You make your way quickly yet cautiously through the fog until you come upon the shell of a ruined tower. The upper floors of the structure have collapsed, leaving heaps of rubble and shattered timber around the tower's base. The feeble light moves through an open doorway on the ground floor, then flickers and goes out.</aside>The light is a will-o'-wisp that enters the ruined tower and becomes invisible, hoping to lure the characters inside to their doom.</p><p>The floor of the tower is made of packed earth. Its interior is desecrated ground. Against the inside wall of the tower, across from the open doorway, is a closed, empty wooden chest.</p><p>If the characters disturb the chest, ${roll(
        "3d6"
      )} zombies erupt from the earthen floor and attack. Once the zombies appear, the will-o'-wisp becomes visible and joins the fray.</p><p><strong>Desecrated Ground:</strong> a detect evil and good spell cast within range reveals the presence of desecrated ground, and undead standing on desecrated ground have advantage on all saving throws. A vial of holy water purifies a 10-foot-square area of desecrated ground when sprinkled on it, and a hallow spell purifies desecrated ground within its area.`;
